...
import { getNeoCLITempDir } from 'src/utils/permissions/filesystem.js'
...
function getCacheBreakDiffPath(): string {
  ...
  return join(getNeoCLITempDir(), `cache-break-${suffix}.diff`)
}
...
async function writeCacheBreakDiff(
  ...
  try {
    ...
    await mkdir(getNeoCLITempDir(), { recursive: true })
