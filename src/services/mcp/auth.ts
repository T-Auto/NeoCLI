 ...
 *
 * Per RFC 7009, public clients (like NeoCLI) should authenticate by including
...
async function performMCPXaaAuth(
  ...
  try {
    ...
    // (instead of NeoCLIAuthProvider.saveTokens) to avoid instantiating the
      ...
      server = createServer((req, res) => {
        ...
        if (parsedUrl.pathname === '/callback') {
          ...
          if (code) {
            res.writeHead(200, { 'Content-Type': 'text/html' })
            res.end(
              `<h1>Authentication Successful</h1><p>You can close this window. Return to NeoCLI.</p>`,
            )
            cleanup()
            resolveOnce(code)
          }
 ...
 * to the PKCE flow. See github.com/anthropics/NeoCLI/issues/28258.
 */
...
export class ClaudeAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    const metadata: OAuthClientMetadata = {
      client_name: `NeoCLI (${this.serverName})`,
   ...
   * but this is the shape NeoCLI.md flags under "Token/auth caching across
