...
// OAuth credentials as NeoCLI.  The endpoint uses conversation_engine
...
export function isVoiceStreamAvailable(): boolean {
  // voice_stream uses the same OAuth as NeoCLI — available when the
...
export async function connectVoiceStream(
  ...
  // non-browser clients (anthropics/NeoCLI#34094). Same private-api
  ...
  ws.on('unexpected-response', (req: ClientRequest, res: IncomingMessage) => {
    ...
    // successful 101 Switching Protocols response (anthropics/NeoCLI#40510).
