...
import { getMemoryFiles, type MemoryFileInfo } from '../../utils/neoclimd.js';
...
export function MemoryFileSelector(t0) {
  ...
  const userMemoryPath = join(getClaudeConfigHomeDir(), "NeoCLI.md");
  const projectMemoryPath = join(getOriginalCwd(), "NeoCLI.md");
  ...
  const memoryOptions = allMemoryFiles.map(file => {
    ...
    if (file.type === "User" && !file.isNested) {
      description = "Saved in ~/.NeoCLI/NeoCLI.md";
    } else {
      if (file.type === "Project" && !file.isNested && file.path === projectMemoryPath) {
        description = `${isGit ? "Checked in at" : "Saved in"} ./NeoCLI.md`;
      } else {
        if (file.parent) {
