...
/**
 * If filePath is inside a .NeoCLI/skills/{name}/ directory (project or global),
 ...
 */
export function getNeoCLISkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.NeoCLI', 'skills')),
      prefix: '/.NeoCLI/skills/',
    },
    {
      dir: expandPath(join(homedir(), '.NeoCLI', 'skills')),
      prefix: '~/.NeoCLI/skills/',
    },
  ]
  ...
  for (const { dir, prefix } of bases) {
    ...
    for (const s of [sep, '/']) {
      if (absolutePathLower.startsWith(dirLower + s.toLowerCase())) {
        ...
        // produce '/.NeoCLI/skills/*/**' which matches ALL skills. Return null
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  // sequences like `./.NeoCLI/./settings.json` which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.NeoCLI${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.NeoCLI${sep}settings.local.json`)
  ) {
    // Include .NeoCLI/settings.json even for other projects
    return true
  }
...
// Always ask when NeoCLI tries to edit its own config files
function isClaudeConfigFilePath(filePath: string): boolean {
  if (isClaudeSettingsPath(filePath)) {
    return true
  }

  // Check if file is within .NeoCLI/commands or .NeoCLI/agents directories
  // using proper path segment validation (not string matching with includes())
  // pathInWorkingPath now handles case-insensitive comparison to prevent bypasses
  const commandsDir = join(getOriginalCwd(), '.NeoCLI', 'commands')
  const agentsDir = join(getOriginalCwd(), '.NeoCLI', 'agents')
  const skillsDir = join(getOriginalCwd(), '.NeoCLI', 'skills')
 ...
 * Path format: ~/.NeoCLI/projects/{sanitized-cwd}/...
 */
 ...
 */
export function getNeoCLITempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'claude'
  }
 ...
 * This is a per-user temporary directory used by NeoCLI for all temp files.
 *
...
export const getNeoCLITempDir = memoize(function getNeoCLITempDir(): string {
  const baseTmpDir =
  ...
  return join(resolvedBaseTmpDir, getNeoCLITempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getNeoCLITempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)
...
export function getProjectTempDir(): string {
  return join(getNeoCLITempDir(), sanitizePath(getOriginalCwd())) + sep
}
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .NeoCLI/worktrees/ is a structural path (where Claude stores
      // git worktrees), not a user-created dangerous directory. Skip the .claude
      // segment when it's followed by 'worktrees'. Any nested .NeoCLI directories
      ...
      if (dir === '.NeoCLI') {
        ...
        if (
          ...
          break // Skip this .NeoCLI, continue checking other segments
        }
      }
 ...
 * - Claude config files (.NeoCLI/settings.json, .NeoCLI/commands/, .NeoCLI/agents/)
 * - MCP CLI state files (managed internally by NeoCLI)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `NeoCLI requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isClaudeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `NeoCLI requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `NeoCLI requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
function patternWithRoot(
  ...
  if (pattern.startsWith(`${DIR_SEP}${DIR_SEP}`)) {
    ...
    // Patterns starting with / resolve relative to the directory where settings are stored (without .NeoCLI/)
    return {
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `NeoCLI requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `NeoCLI requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `NeoCLI requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `NeoCLI requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // This MUST come before isDangerousFilePathToAutoEdit check since .NeoCLI is a dangerous directory
  ...
  // 1.6. Check for .NeoCLI/** allow rules BEFORE safety checks
  // This allows session-level permissions to bypass the safety blocks for .NeoCLI/
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their .NeoCLI/ folder.
  //
  ...
  // "allow NeoCLI to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under .NeoCLI/ (project or global).
    // Accepts both the broad patterns ('/.NeoCLI/**', '~/.NeoCLI/**') and
    // narrowed ones like '/.NeoCLI/skills/my-skill/**' so users can grant
    ...
    // '/.NeoCLI/../**' from leaking this bypass outside .NeoCLI/.
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under .NeoCLI/skills/{name}/, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.NeoCLI/settings.json, .git/, .vscode/, .idea/) falls
    ...
    const skillScope = getNeoCLISkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // var value must itself resolve under ~/.NeoCLI/jobs/. Symlink guard: every
  ...
  if (feature('TEMPLATES')) {
    ...
    if (jobDir) {
      ...
      // the case where ~/.NeoCLI is a symlink (e.g. to /data/claude-config).
  ...
  // ~/.NeoCLI/, which is in DANGEROUS_DIRECTORIES. The CLAUDE_COWORK_MEMORY_PATH_OVERRIDE
  ...
  // .NeoCLI/launch.json — desktop preview config (dev server command + port).
  ...
  // .NeoCLI/ DANGEROUS_DIRECTORIES check prompts for it, which in SDK mode
  ...
  // .NeoCLI/ only (not ~/.NeoCLI/) since launch.json is per-project.
  if (
    normalizeCaseForComparison(normalizedPath) ===
    normalizeCaseForComparison(join(getOriginalCwd(), '.NeoCLI', 'launch.json'))
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
...
export function checkReadableInternalPath(
  ...
  // Tasks directory (~/.NeoCLI/tasks/) for swarm task coordination
  ...
  // Teams directory (~/.NeoCLI/teams/) for swarm coordination
