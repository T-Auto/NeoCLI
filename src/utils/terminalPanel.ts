 ...
 * NeoCLI while the shell keeps running. Next toggle re-attaches to the
 * same session.
 *
 ...
 * Uses a unique socket per NeoCLI instance (based on session ID)
...
class TerminalPanel {
  ...
  private createSession(): boolean {
    ...
    // Bind Meta+J (toggles back to NeoCLI from inside the terminal)
    ...
    spawnSync('tmux', [
      ...
      'set-option', '-g', 'status-right', ' Alt+J to return to NeoCLI ', ';',
