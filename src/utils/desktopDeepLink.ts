...
/**
 * Builds a deep link URL for NeoCLI Desktop to resume a CLI session.
...
/**
 * Check if NeoCLI Desktop app is installed.
 * On macOS, checks for /Applications/NeoCLI.app.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    ...
    return pathExists('/Applications/NeoCLI.app')
...
/**
 * Detect the installed NeoCLI Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/NeoCLI.app/Contents/Info.plist',
...
/**
 * Build and open a deep link to resume the current session in NeoCLI Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'NeoCLI Desktop is not installed. Install it from https://claude.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open NeoCLI Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
