 ...
 * user-controlled files (~/.NeoCLI/settings.json and ~/.NeoCLI/.config.json),
 ...
 *
 * We read from global config (~/.NeoCLI/.config.json) and user settings
 * (~/.NeoCLI/settings.json). These are user-controlled files that don't
...
function getExtraCertsPathFromConfig(): string | undefined {
  try {
    ...
    // Only read from user-controlled settings (~/.NeoCLI/settings.json),
