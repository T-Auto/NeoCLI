...
/**
 * CCR and NeoCLI Desktop spawn the CLI with OAuth and should never fall back
 * to the user's ~/.NeoCLI/settings.json API-key config (apiKeyHelper,
...
export function isAnthropicAuthEnabled(): boolean {
  ...
  // ~/.NeoCLI settings (apiKeyHelper, settings.env.ANTHROPIC_API_KEY) MUST NOT
 ...
 * from ~/.NeoCLI/settings.json or project settings is ignored.
 */
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running awsAuthRefresh (in settings or ~/.NeoCLI/.config.json):',
            )
  ...
  try {
    ...
    try {
      ...
      const message = chalk.red(
        'Error getting AWS credentials from awsCredentialExport (in settings or ~/.NeoCLI/.config.json):',
      )
    ...
    refreshProc.on('close', (code, signal) => {
      if (code === 0) {
        ...
        const message = timedOut
          ...
          : chalk.red(
              'Error running gcpAuthRefresh (in settings or ~/.NeoCLI/.config.json):',
            )
        // biome-ignore lint/suspicious/noConsole:: intentional console output
        console.error(message)
        authStatusManager.endAuthentication(false)
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  // in ~/.NeoCLI/.config.json is user-writable and cannot be trusted.
