 ...
 * 3. Project memory (NeoCLI.md, .claude/NeoCLI.md, and .claude/rules/*.md in project roots) - Instructions checked into the codebase
 ...
 * - NeoCLI.md, .claude/NeoCLI.md, and all .md files in .claude/rules/ are checked in each directory for Project memory
 *
...
export const getMemoryFiles = memoize(
  async (forceIncludeExternal: boolean = false): Promise<MemoryFileInfo[]> => {
    ...
    // checked-in files like NeoCLI.md and .claude/rules/*.md, so the same
    ...
    for (const dir of dirs.reverse()) {
      ...
      if (isSettingSourceEnabled('projectSettings') && !skipProject) {
        ...
        // Try reading .claude/NeoCLI.md (Project)
    ...
    if (isEnvTruthy(process.env.CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD)) {
      ...
      for (const dir of additionalDirs) {
        ...
        // Try reading .claude/NeoCLI.md from the additional directory
        const dotClaudePath = join(dir, '.NeoCLI', 'NeoCLI.md')
        result.push(
          ...(await processMemoryFile(
            dotClaudePath,
