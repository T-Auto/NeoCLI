 ...
 * env vars that the remote's ~/.NeoCLI settings.env MUST NOT clobber (see
 ...
 * user's ~/.NeoCLI/settings.json can't redirect requests away from the
 ...
 *
 * - userSettings (~/.NeoCLI/settings.json): controlled by the user, not project-specific
...
export function applySafeConfigEnvironmentVariables(): void {
  ...
  // Global config (~/.NeoCLI/.config.json) is user-controlled. In CCD mode,
  ...
  // doesn't get clobbered by ~/.NeoCLI/settings.json env (gh#217). policy/flag
