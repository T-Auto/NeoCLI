...
export function isRunningFromLocalInstallation(): boolean {
  ...
  return execPath.includes('/.NeoCLI/local/node_modules/') || execPath.includes('/.claude/local/node_modules/')
}
...
/**
 * Install or update NeoCLI CLI package in the local directory
  ...
  try {
    ...
    if (result.code !== 0) {
      const error = new Error(
        `Failed to install NeoCLI CLI package: ${result.stderr}`,
      )
      logError(error)
