...
const APP_NAME = 'NeoCLI URL Handler'
const DESKTOP_FILE_NAME = 'claude-code-url-handler.desktop'
const MACOS_APP_NAME = 'NeoCLI URL Handler.app'
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>NeoCLI Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for NeoCLI
 ...
 * a cached flag in ~/.NeoCLI/.config.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.claude (per-machine, not synced) rather than ~/.NeoCLI/.config.json (can sync).
