 ...
 * notice which directory — and therefore which NeoCLI.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which NeoCLI.md
 ...
 * and whether its NeoCLI.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — NeoCLI.md may be stale' : ''}`,
    )
  }
