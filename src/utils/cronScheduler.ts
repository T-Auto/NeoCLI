// Non-React scheduler core for .NeoCLI/scheduled_tasks.json.
...
type CronSchedulerOptions = {
  ...
  /**
   * Directory containing .NeoCLI/scheduled_tasks.json. When provided, the
...
export function createCronScheduler(
  ...
  function check() {
    ...
    // exists to stop two NeoCLI sessions in the same cwd from double-firing
...
export function buildMissedTaskNotification(missed: CronTask[]): string {
  ...
  const header =
    `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while NeoCLI was not running. ` +
    `${plural ? 'They have' : 'It has'} already been removed from .NeoCLI/scheduled_tasks.json.\n\n` +
