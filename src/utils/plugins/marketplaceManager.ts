/**
 * Marketplace manager for NeoCLI plugins
 *
 ...
 *
 * Reads the configuration file at ~/.NeoCLI/plugins/known_marketplaces.json
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a NeoCLI bug, shouldn't hit the error file.
    return {}
  }
}
 ...
 *
 * Writes the configuration to ~/.NeoCLI/plugins/known_marketplaces.json,
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'NeoCLI-Plugin-Manager',
  }
 ...
 * - GitHub: Clones repo and looks for .NeoCLI-plugin/marketplace.json
    ...
    switch (source.source) {
      ...
      case 'github': {
        ...
        marketplacePath = join(
          temporaryCachePath,
          source.path || '.NeoCLI-plugin/marketplace.json',
        )
        break
      }
      ...
      case 'file': {
        ...
        // File sources point to .NeoCLI-plugin/marketplace.json, so the marketplace
        // root is two directories up (parent of .NeoCLI-plugin/)
      ...
      case 'directory': {
        // For directories, look for .NeoCLI-plugin/marketplace.json
        // Resolve to absolute so error messages show the actual path checked
        // (legacy known_marketplaces.json entries may have relative paths)
        const absPath = resolve(source.path)
        marketplacePath = join(absPath, '.NeoCLI-plugin', 'marketplace.json')
 ...
 * The configuration is saved to ~/.NeoCLI/plugins/known_marketplaces.json.
 *
...
async function readCachedMarketplace(
  ...
  // For git-sourced directories, the manifest lives at .NeoCLI-plugin/marketplace.json.
  ...
  const nestedPath = join(installLocation, '.NeoCLI-plugin', 'marketplace.json')
  try {
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `NeoCLI version. Run 'claude marketplace remove ${name}' and ` +
