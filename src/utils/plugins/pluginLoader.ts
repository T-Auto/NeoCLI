 ...
 *
 * This module is responsible for discovering, loading, and validating NeoCLI plugins
 ...
 * Format: ~/.NeoCLI/plugins/cache/{marketplace}/{plugin}/{version}/
 *
 * @param pluginId - Plugin identifier in format "name@marketplace"
 * @param version - Version string (semver, git SHA, etc.)
 ...
 * Format: ~/.NeoCLI/plugins/cache/{plugin-name}/
 *
...
export async function cachePlugin(
  ...
  const manifestPath = join(tempPath, '.NeoCLI-plugin', 'plugin.json')
 ...
 * @param source - Source description for default manifest (e.g., "git:repo" or ".NeoCLI-plugin/name")
 ...
 * @param source - Source identifier (e.g., "git:repo", ".NeoCLI-plugin/my-plugin")
...
export async function createPluginFromPath(
  ...
  const manifestPath = join(pluginPath, '.NeoCLI-plugin', 'plugin.json')
  ...
  const plugin: LoadedPlugin = {
    ...
    source, // Source identifier (e.g., "git:repo" or ".NeoCLI-plugin/name")
 ...
 * - After modifying .NeoCLI-plugin/ directory (for export)
