...
/**
 * Generates NeoCLI specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and NeoCLI content
 ...
 *
 * This function is a critical part of NeoCLI CLI's shell integration strategy. It:
 *
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with NeoCLI defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - NeoCLI home: ${getClaudeConfigHomeDir()}`)
