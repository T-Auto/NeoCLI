...
export function getAllHooks(appState: AppState): IndividualHookConfig[] {
  ...
  if (!restrictedToManagedOnly) {
    ...
    // both resolve to ~/.NeoCLI/settings.json)
...
export function hookSourceDescriptionDisplayString(source: HookSource): string {
  switch (source) {
    case 'userSettings':
      return 'User settings (~/.NeoCLI/settings.json)'
    case 'projectSettings':
      return 'Project settings (.NeoCLI/settings.json)'
    case 'localSettings':
      return 'Local settings (.NeoCLI/settings.local.json)'
    case 'pluginHook':
      ...
      // e.g., "Plugin hooks (~/.NeoCLI/plugins/repos/source/example-plugin/example-plugin/hooks/hooks.json)"
      return 'Plugin hooks (~/.NeoCLI/plugins/*/hooks/hooks.json)'
    case 'sessionHook':
      return 'Session hooks (in-memory, temporary)'
    case 'builtinHook':
      return 'Built-in hooks (registered internally by NeoCLI)'
    default:
