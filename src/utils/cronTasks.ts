// Scheduled prompts, stored in <project>/.NeoCLI/scheduled_tasks.json.
//
...
const CRON_FILE_REL = join('.NeoCLI', 'scheduled_tasks.json')
...
/**
 * Read and parse .NeoCLI/scheduled_tasks.json. Returns an empty task list if the file
...
export async function writeCronTasks(
  ...
  await mkdir(join(root, '.NeoCLI'), { recursive: true })
 ...
 * written to .NeoCLI/scheduled_tasks.json and dies with the process. The
