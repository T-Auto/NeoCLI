 ...
 *
 * Shared by buildMemoryPrompt and neoclimd getMemoryFiles (previously
 ...
 * (~/.NeoCLI/projects/<slug>/memory/) is created in one call with no
 ...
 * Used by agent memory (which has no getNeoCLIMds() equivalent).
 */
 ...
 * files + MEMORY.md. MEMORY.md is still loaded into context (via neoclimd.ts)
