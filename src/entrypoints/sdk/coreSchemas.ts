...
export const ThinkingAdaptiveSchema = lazySchema(() =>
  z
    ...
    .describe('NeoCLI decides when and how much to think (Opus 4.6+).'),
)
...
export const ThinkingConfigSchema = lazySchema(() =>
  z
    ...
    .describe(
      "Controls NeoCLI's thinking/reasoning behavior. When set, takes precedence over the deprecated maxThinkingTokens.",
    ),
)
  ...
  z
    .object({
      ...
      supportsAdaptiveThinking: z
        ...
        .optional()
        .describe(
          'Whether this model supports adaptive thinking (NeoCLI decides when and how much to think)',
        ),
      supportsFastMode: z
        .boolean()
        .optional()
  ...
  z
    .object({
      ...
      memory: z
        ...
        .optional()
        .describe(
          "Scope for auto-loading agent memory files. 'user' - ~/.NeoCLI/agent-memory/<agentType>/, 'project' - .NeoCLI/agent-memory/<agentType>/, 'local' - .NeoCLI/agent-memory-local/<agentType>/",
        ),
      effort: z
  ...
  z
    ...
    .describe(
      'Source for loading filesystem-based settings. ' +
        "'user' - Global user settings (~/.NeoCLI/settings.json). " +
        "'project' - Project settings (.NeoCLI/settings.json). " +
        "'local' - Local settings (.NeoCLI/settings.local.json).",
    ),
)
