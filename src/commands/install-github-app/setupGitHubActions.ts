...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: A NeoCLI workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    const helpText =
      ...
      '· For manual setup → Visit: https://github.com/anthropics/NeoCLI-action'
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude')) {
        workflows.push({
          ...
          message: 'NeoCLI PR Assistant workflow',
        })
      }
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          ...
          message: 'NeoCLI Review workflow',
        })
      }
