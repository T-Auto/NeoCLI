...
import { getExternalNeoCLIMdIncludes, getMemoryFiles, shouldShowNeoCLIMdExternalIncludesWarning } from './utils/neoclimd.js';
...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (await shouldShowNeoCLIMdExternalIncludesWarning()) {
      const externalIncludes = getExternalNeoCLIMdIncludes(await getMemoryFiles(true));
      const {
        NeoCLIMdExternalIncludesDialog
      } = await import('./components/NeoCLIMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <NeoCLIMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
